...
This is the one responsible for fetching the extracted bills from CC&B for the consumption of [**Bill Stager**](https://github.com/sbamihan/build/tree/master/eBill/bill-stager).

Procedures:
1.	Fetches extracted bills from CC&B based on parameters provided by **Bill Stager**.

    Should accept REST call at GET `/bills/search/findByBatchNoAndAcctNo?batchNo={{batchNo}}&acctNo={{acctNo}}` endpoint and returns bill information similar to this.
